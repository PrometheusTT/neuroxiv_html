...
import HumanNeuronDetail from '@/components/human/HumanNeuronDetail.vue'
...
import {
  ...
  AI_RAG,
  getSearchCondition
} from '@/request/apis/human/Neuron'
...
export default class Container extends Vue {
  @Ref('neuronDetail') readonly neuronDetail!: HumanNeuronDetail
   ...
   */
    ...
    private async uploadNeuronHandler (param: any) {
      try {
        ...
        // 处理请求结果
        // eslint-disable-next-line camelcase
        requestInstance.start().then((neuronInfo: { viewer_info: any }) => {
          this.neuronDetail.neuronInfo.clearReconstruction()
          this.neuronDetail.neuronInfo.hideSoma()
          this.neuronDetail.neuronInfo.isUploadData = false
          this.$nextTick().then(() => {
   ...
   */
    ...
    //   if (switchTab && this.neuronDetail.selectedTab !== 'humanMultiNeuronsViewer') {
    //     this.neuronDetail.selectedTab = 'humanMultiNeuronsViewer'
    //     await this.$nextTick()
    //   }
    //   await this.$nextTick()
    //   if (this.neuronDetail.selectedTab === 'humanMultiNeuronsViewer') {
    //     let dendriteData = {
    ...
    //       }
    //     }
    //     if (this.neuronDetail.humanMultiNeuronsViewer.neuronScene.checkLoadComponent(dendriteData) ||
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.checkLoadComponent(axonData) || this.neuronDetail.humanMultiNeuronsViewer.neuronScene.checkLoadComponent(apicalData) || this.neuronDetail.humanMultiNeuronsViewer.neuronScene.checkLoadComponent(localData) ||
    //         !neuronDetail.selected) {
    //       if (neuronDetail['has_dendrite'] && this.neuronDetail.humanMultiNeuronsViewer.showAllBasal) {
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(dendriteData, neuronDetail.selected)
    //       }
    //       if (neuronDetail['has_axon'] && this.neuronDetail.humanMultiNeuronsViewer.showAllAxon) {
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(axonData, neuronDetail.selected)
    //       }
    //       if (neuronDetail['has_apical'] && this.neuronDetail.humanMultiNeuronsViewer.showAllApical) {
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(apicalData, neuronDetail.selected)
    //       }
    //       if (neuronDetail['has_local'] && this.neuronDetail.humanMultiNeuronsViewer.showAllBasal) {
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(localData, neuronDetail.selected)
    //       }
    //     } else {
    //       const neuronInfo = await getNeuronInfo(document.body, neuronDetail.id, this.$store.state.atlas).start()
    //       console.log(neuronInfo)
    //       this.neuronDetail.humanMultiNeuronsViewer.neuronScene.multiViewerSomaPos.set(neuronDetail.id, neuronInfo.soma)
    ...
    //         await this.neuronDetail.humanMultiNeuronsViewer.neuronScene.loadObj(dendriteData)
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(dendriteData, this.neuronDetail.humanMultiNeuronsViewer.showAllBasal)
    //       }
    //       if (neuronDetail['has_axon']) {
    //         await this.neuronDetail.humanMultiNeuronsViewer.neuronScene.loadObj(axonData)
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(axonData, this.neuronDetail.humanMultiNeuronsViewer.showAllAxon)
    //       }
    //       if (neuronDetail['has_apical']) {
    //         await this.neuronDetail.humanMultiNeuronsViewer.neuronScene.loadObj(apicalData)
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(apicalData, this.neuronDetail.humanMultiNeuronsViewer.showAllApical)
    //       }
    //       if (neuronDetail['has_local']) {
    //         await this.neuronDetail.humanMultiNeuronsViewer.neuronScene.loadObj(localData)
    //         this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(localData, this.neuronDetail.humanMultiNeuronsViewer.showAllBasal)
    //       }
    //     }
    //     await this.setVisualizedSoma()
    //   }
    ...
    public async checkNeuron (neuronDetail: any, switchTab: boolean = false) {
      ...
      const neuronScene = this.neuronDetail.humanMultiNeuronsViewer.neuronScene
      ...
      if ([dendriteData, apicalData, axonData, localData].some(checkLoadComponent) || !neuronDetail.selected) {
        ...
        await Promise.all([
          loadAndSetVisible(dendriteData, neuronDetail['has_dendrite'], this.neuronDetail.humanMultiNeuronsViewer.showAllBasal),
          loadAndSetVisible(axonData, neuronDetail['has_axon'], this.neuronDetail.humanMultiNeuronsViewer.showAllAxon),
          loadAndSetVisible(apicalData, neuronDetail['has_apical'], this.neuronDetail.humanMultiNeuronsViewer.showAllApical),
          loadAndSetVisible(localData, neuronDetail['has_local'], this.neuronDetail.humanMultiNeuronsViewer.showAllBasal)
        ])
      }
   ...
   */
    public async viewNeurons () {
      ...
      this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setAllNeuronsVisible(false)
   ...
   */
    public async setVisualizedSoma () {
      ...
      // 显示选中的神经元
      neuronsDetail.forEach((neuronDetail: any) => {
        this.neuronDetail.humanMultiNeuronsViewer.neuronScene.showMultiViewerSomaBall(neuronDetail.id, 100, this.neuronDetail.humanMultiNeuronsViewer.showAllSoma)
      })

      // 清除Map中未被选中的神经元
      this.neuronDetail.humanMultiNeuronsViewer.neuronScene.multiViewerSoma.forEach((value, key) => {
        if (!selectedIds.has(key)) {
          this.neuronDetail.humanMultiNeuronsViewer.neuronScene.unloadMultiViewerSomaBalls(key)
          this.neuronDetail.humanMultiNeuronsViewer.neuronScene.multiViewerSoma.delete(key)
        }
      })
    }
    public async setVisualizedAxon ($event: any) {
      ...
      neuronsDetail.forEach((neuronDetail: any) => {
        ...
        if (this.neuronDetail.humanMultiNeuronsViewer.neuronScene.checkLoadComponent(axonData)) {
          if (neuronDetail['has_axon']) {
            this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(axonData, this.neuronDetail.humanMultiNeuronsViewer.showAllAxon)
          }
        }
    ...
    public async setVisualizedBasal () {
      ...
      neuronsDetail.forEach((neuronDetail: any) => {
        ...
        if (this.neuronDetail.humanMultiNeuronsViewer.neuronScene.checkLoadComponent(dendriteData) || this.neuronDetail.humanMultiNeuronsViewer.neuronScene.checkLoadComponent(localData)) {
          if (neuronDetail['has_dendrite']) {
            this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(dendriteData, this.neuronDetail.humanMultiNeuronsViewer.showAllBasal)
          }
          if (neuronDetail['has_local']) {
    ...
    public async setVisualizedApical () {
      ...
      neuronsDetail.forEach((neuronDetail: any) => {
        ...
        if (this.neuronDetail.humanMultiNeuronsViewer.neuronScene.checkLoadComponent(apicalData)) {
          if (neuronDetail['has_dendrite']) {
            this.neuronDetail.humanMultiNeuronsViewer.neuronScene.setComponentVisible(apicalData, this.neuronDetail.humanMultiNeuronsViewer.showAllApical)
          }
        }
    ...
    private changeResolution () {
      let neuronsDetail = this.neuronList.getSelectedItems()
      const showAllAxon = this.neuronDetail.humanMultiNeuronsViewer.showAllAxon
      const showAllApical = this.neuronDetail.humanMultiNeuronsViewer.showAllApical
      const showAllBasal = this.neuronDetail.humanMultiNeuronsViewer.showAllBasal
      ...
      neuronsDetail.forEach((neuronDetail: any) => {
        ...
        this.neuronDetail.humanMultiNeuronsViewer.buttonText = this.useRawObj ? 'Switch to downsampled morpho' : 'Switch to raw morpho'
        ...
        if (neuronDetail['has_axon']) {
          ...
          console.log(showAllAxon)
          this.neuronDetail.humanMultiNeuronsViewer.neuronScene.loadObjSetVisible(axon, showAllAxon)
        }

        if (neuronDetail['has_dendrite']) {
          this.neuronDetail.humanMultiNeuronsViewer.neuronScene.loadObjSetVisible(dendrite, showAllBasal)
        }

        if (neuronDetail['has_apical']) {
          this.neuronDetail.humanMultiNeuronsViewer.neuronScene.loadObjSetVisible(apical, showAllApical)
        }
      })
   ...
   */
    ...
    public async showNeuronMap () {
      this.neuronDetail.humanMultiNeuronsViewer.neuronScene.showMap(10)
    }
