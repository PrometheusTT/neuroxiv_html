  ...
  <PageWithHeaderFooter header-title="Sange The Master">
    <div class="login-options-container">
      <transition :name="!selectLoginOption ? 'slide-fade-opposite' : 'slide-fade'">
        <div
          v-if="!selectLoginOption"
          key="options"
          class="login-options-box transition-item abs-full vertical-middle"
        >
          <LoginOptions
            class="centered"
            @selectLoginOption="loginOptionChangeHandler($event.detail)"
          />
        </div>
        <div
          v-if="selectLoginOption === 'email'"
          key="email"
          class="mail-login transition-item abs-full vertical-middle"
        >
          <div class="centered">
            <MailLoginForm />
            <p class="back-to-options">
              <span
                class="back-btn"
                @click="selectLoginOption = ''"
              ><i class="el-icon-arrow-left" />其他登录方式</span>
              <a
                href="#"
                class="forget-link"
              >忘记密码?</a>
